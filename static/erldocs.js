...
$(window).load(function () {
    ...
    if ($('#mod_list_tbody').length > 0) {
        ...
        $("#mod_list_tbody").html(_html.join(""));
        $("#mod_count").html(mod_count);
        $('#mod_list_tbody tr').each(function () {
            $(this).hover(
                function () {
    ...
    if ($('#func_list_tbody').length > 0) {
        ...
        function func_pages(total, per_page, current) {
            ...
            _html.push("</ul>");
            $("#func_pages").html(_html.join(""));
            $("#func_pages li").each(function () {
                if ($(this).attr("page")) {
        ...
        function draw_func_list() {
            ...
            var per_page = parseInt($("#func_per_page").val());
            if (!per_page || per_page < 1) {
                per_page = 20;
            }
            var current = parseInt($("#func_per_page").attr("current"));
            if (!current || current < 1) {
                current = 1;
            }
            start_count = (current - 1) * per_page
            ...
            $("#func_list_tbody").html(_html.join(""));
            $("#func_count").html(func_count);
            func_pages(func_count, per_page, current);
            $("#func_per_page").val(per_page);
            $('#func_list_tbody tr').each(function () {
                $(this).hover(
                    function () {
                        $(this).css("background-color", "#FAFAD1");
                    },
        ...
        for (var i = 0; i < mod_list_sel_len; i++) {
            mod_list_sel.push("<option value='", mod_list[i], "'>", mod_list[i], "</option>");
        }
        $('#mod_list_sel').html(mod_list_sel.join(""));
        $('#mod_list_sel').change(function () {
